...
const supabaseKey = cfg.supabaseAnonKey || cfg.SUPABASE_KEY;
const configured = supabaseUrl && supabaseKey && !supabaseUrl.startsWith('YOUR_');
const db = configured ? window.supabase.createClient(supabaseUrl, supabaseKey) : null;
...
function localBackupKey(){ return `poundBudgetTracker:${selectedMonth}`; }
function saveLocalBackup(){ localStorage.setItem(localBackupKey(), JSON.stringify(state)); }
async function cloudLoad(){
  if(!db || !currentUser) return null;
  setSyncStatus('Syncing…');
  const {data,error}=await db.from('budget_months').select('data').eq('user_id',currentUser.id).eq('month',selectedMonth).maybeSingle();
  if(error){ setSyncStatus('Sync error','error'); console.error(error); return null; }
  setSyncStatus('Synced','ok');
  return data?.data || null;
}
async function cloudSaveNow(){
  if(!db || !currentUser || loadingCloud) return;
  saveLocalBackup();
  setSyncStatus('Saving…');
  const {error}=await db.from('budget_months').upsert({user_id:currentUser.id,month:selectedMonth,data:state,updated_at:new Date().toISOString()},{onConflict:'user_id,month'});
  if(error){ setSyncStatus('Saved locally · cloud error','error'); console.error(error); } else setSyncStatus('Synced','ok');
}
function queueCloudSave(){
  saveLocalBackup();
  clearTimeout(saveTimer);
  saveTimer=setTimeout(cloudSaveNow,500);
}
async function loadSelectedMonth(){
  loadingCloud=true;
  const cloud=await cloudLoad();
  if(cloud){ state=cloud; saveLocalBackup(); }
  else {
    try{ const local=localStorage.getItem(localBackupKey()); state=local?JSON.parse(local):blankState(); }catch(e){ state=blankState(); }
    if(currentUser) await db.from('budget_months').upsert({user_id:currentUser.id,month:selectedMonth,data:state,updated_at:new Date().toISOString()},{onConflict:'user_id,month'});
  }
  loadingCloud=false;
  renderAll();
}
...
async function switchView(view){
  ...
  const titles={dashboard:'Budget Dashboard',budget:'Monthly Budget',savings:'Savings Goals',review:'Monthly Review',yearly:'Yearly Review'};
  setText('pageTitle',titles[view]);
  window.scrollTo({top:0,behavior:'smooth'});
  if(view==='yearly') await loadYearlyReview();
}
...
document.addEventListener('click',e=>{
  ...
  if(e.target.id==='resetBtn'){
    if(confirm(`Reset all saved data for ${prettyMonth(selectedMonth)}?`)){
      localStorage.removeItem(localBackupKey());state=blankState();queueCloudSave();renderAll();
    }
  }
...
document.getElementById('yearSelect')?.addEventListener('change',loadYearlyReview);

function populateYearSelect(){
  const select=document.getElementById('yearSelect'); if(!select)return;
  const currentYear=new Date().getFullYear();
  const selected=Number(select.value)||Number(selectedMonth.slice(0,4))||currentYear;
  select.innerHTML='';
  for(let y=currentYear+1;y>=currentYear-8;y--){
    const o=document.createElement('option');o.value=String(y);o.textContent=String(y);if(y===selected)o.selected=true;select.appendChild(o);
  }
}
function stateActualTotals(st){
  const income=sum((st?.actualIncome||[]).map(x=>x.amount));
  const spend=sum((st?.categories||[]).flatMap(c=>(c.items||[]).map(i=>i.actual)));
  return {income,spend};
}
async function loadYearlyReview(){
  if(!db||!currentUser)return;
  populateYearSelect();
  const year=document.getElementById('yearSelect')?.value || selectedMonth.slice(0,4);
  const chart=document.getElementById('yearMonthlyChart');
  if(chart) chart.innerHTML='<p class="year-loading">Loading your year…</p>';
  const {data,error}=await db.from('budget_months').select('month,data').eq('user_id',currentUser.id).like('month',`${year}-%`).order('month');
  if(error){ if(chart) chart.innerHTML='<p class="negative">Could not load yearly data.</p>'; console.error(error); return; }
  renderYearlyReview(year,data||[]);
}
function renderYearlyReview(year,rows){
  const byMonth=new Map(rows.map(r=>[r.month,r.data]));
  const monthRows=[]; const categories={}; let totalIncome=0,totalSpent=0,recorded=0;
  for(let m=1;m<=12;m++){
    const key=`${year}-${String(m).padStart(2,'0')}`, st=byMonth.get(key);
    let income=0,spend=0;
    if(st){
      ({income,spend}=stateActualTotals(st)); recorded++;
      (st.categories||[]).forEach(c=>{ categories[c.name]=(categories[c.name]||0)+sum((c.items||[]).map(i=>i.actual)); });
    }
    totalIncome+=income; totalSpent+=spend;
    monthRows.push({key,label:new Date(Number(year),m-1,1).toLocaleDateString('en-GB',{month:'short'}),income,spend});
  }
  setText('yearIncome',money.format(totalIncome));setText('yearSpent',money.format(totalSpent));
  setText('yearAverage',money.format(recorded?totalSpent/recorded:0));setText('yearBalance',money.format(totalIncome-totalSpent));
  styleSigned(document.getElementById('yearBalance'),totalIncome-totalSpent);
  setText('yearMonthsCount',`${recorded} month${recorded===1?'':'s'}`);setText('yearRecordedMonths',String(recorded));
  const highest=monthRows.reduce((a,b)=>b.spend>a.spend?b:a,{spend:-1,label:'—'});
  setText('yearHighestMonth',highest.spend>0?`${highest.label} · ${money.format(highest.spend)}`:'—');
  const catEntries=Object.entries(categories).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
  setText('yearHighestCategory',catEntries.length?`${catEntries[0][0]} · ${money.format(catEntries[0][1])}`:'—');
  const maxSpend=Math.max(...monthRows.map(x=>x.spend),1);
  const chart=document.getElementById('yearMonthlyChart');chart.innerHTML='';
  monthRows.forEach(x=>{const row=document.createElement('div');row.className='year-bar-row';row.innerHTML=`<span>${x.label}</span><div class="year-bar-track"><div class="year-bar" style="width:${x.spend/maxSpend*100}%"></div></div><strong>${money.format(x.spend)}</strong>`;chart.appendChild(row);});
  const catBox=document.getElementById('yearCategoryBreakdown');catBox.innerHTML='';
  if(!catEntries.length){catBox.innerHTML='<p class="year-loading">No spending recorded for this year yet.</p>';}
  else { const maxCat=catEntries[0][1]||1; catEntries.forEach(([name,value])=>{const row=document.createElement('div');row.className='year-cat-row';row.innerHTML=`<div><strong>${escapeHtml(name)}</strong><span>${money.format(value)}</span></div><div class="year-cat-track"><div style="width:${value/maxCat*100}%"></div></div>`;catBox.appendChild(row);}); }
}
...
async function initAuth(){
  ...
  if(!configured){ msg.textContent='Setup needed: add your Supabase URL and publishable key to config.js.'; setSyncStatus('Setup needed','error'); return; }
  const {data:{session}}=await db.auth.getSession();
  currentUser=session?.user||null;
  if(currentUser){ gate.classList.add('hidden'); await loadSelectedMonth(); }
  else { gate.classList.remove('hidden'); setSyncStatus('Not signed in'); }
...
async function signIn(){
  ...
  if(!configured){msg.textContent='Add your Supabase settings to config.js first.';return;}
...
async function signUp(){
  const email=document.getElementById('authEmail').value.trim(), password=document.getElementById('authPassword').value;
  const msg=document.getElementById('authMessage'); msg.textContent='';
  if(password.length<6){msg.textContent='Choose a password with at least 6 characters.';return;}
